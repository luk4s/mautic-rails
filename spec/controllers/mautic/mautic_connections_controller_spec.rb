    ...
    describe "POST #create" do
      context "with valid params" do
        it "creates a new MauticConnection" do
          expect {
            post :create, params: { use_route: 'mautic', mautic_connection: FactoryBot.attributes_for(:mautic_connection) }
          }.to change(MauticConnection, :count).by(1)
        end

        it "redirects to the created mautic_connection" do
          post :create, params: { use_route: 'mautic', mautic_connection: FactoryBot.attributes_for(:mautic_connection) }
          expect(response).to redirect_to(MauticConnection.last)
        end
      end

      context "with invalid params" do
        it "returns a success response (i.e. to display the 'new' template)" do
          post :create, params: { use_route: 'mautic', mautic_connection: { url: "xxx" } }
          expect(response).to be_success
        end
      end
    end
    ...
    describe "PUT #update" do
      context "with valid params" do
        ...
        it "updates the requested mautic_connection" do
          put :update, params: { use_route: 'mautic', id: mautic_connection.to_param, mautic_connection: { url: "https://newurl.com" } }
      ...
      context "with invalid params" do
        it "returns a success response (i.e. to display the 'edit' template)" do
          put :update, params: { use_route: 'mautic', id: mautic_connection.to_param, mautic_connection: { url: "", secret: "" } }
          expect(response).to be_success
        end
      end
    end
